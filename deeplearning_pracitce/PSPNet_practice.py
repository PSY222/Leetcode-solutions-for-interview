...
class PSPNet(nn.Module):
    def __init__(self,n_classes):
        ...
        self.pyramid_pooling = PyramidPooling(in_channels = 2048, pool_sizes=[6,3,2,1], height=img_size, n_classes=n_classes)
        self.decode_feature = DecodePSPFeature(height=img_size, width=img_size,n_classes=n_classes)
        self.aux = AuxiliaryPlayers(in_channels=1024, height=img_size, width=img_size, n_classes = n_classes)
...
class conv2DBatchNormRelu(nn.Module):
    def __init__(self,in_channels, out_channels, kernel_size, stride, padding,dilation,bias):
        ...
        self.cbnr_2 = conv2DBatchNormRelu(in_channels, kernel_size, stride,padding,dilation,bias)
        in_channels, out_channels, kernel_size, stride, padding, dilation, bias = 64, 128,3,1,1,1, False
        self.cbnr_3 = conv2DBatchNormRelu(in_channels, kernel_size, stride,padding,dilation,bias)
...
class bottleNeckPSP(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, stride, dilation):
        ...
        self.cb_residual = conv2DBatchNorm(in_channels, out_channels, kernel_size =1, stride=1, stride=stride, padding =0, dilation=1, bias=False)
...
class bottleNeckIdentifyPSP(nn.Module):
    def __init__(self,in_channels, mid_channels, out_channels, stride, dilation):
        ...
        self.cb_residual = conv2DBatchNorm(in_channels, out_channels, kernel_size=1, strides=stride, padding=0, dilation=1,bias=False)
        self.relu = nn.ReLU(inplace=True)
...
class PyramidPooling(nn.Module):
    ...
    def forward(self,x):
        ...
        out3 = F.interpolate(out1,size=(self.height,self.width),mode="bilinear",align_corners=True)
